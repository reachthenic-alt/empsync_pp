          ...
          <tr v-for="leave in filteredLeaves" :key="leave.id" class="border-b border-zinc-100 dark:border-zinc-800 last:border-0 hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
            ...
            <td class="px-4 py-3 text-right">
              <div v-if="leave.status === 'pending'" class="flex justify-end gap-2">
                <button @click="updateStatus(leave, 'approved')" class="text-xs px-2.5 py-1 rounded-md bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 hover:bg-green-100 font-medium transition-colors">
                  Approve
                </button>
                <button @click="updateStatus(leave, 'rejected')" class="text-xs px-2.5 py-1 rounded-md bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 hover:bg-red-100 font-medium transition-colors">
                  Reject
                </button>
              </div>
