<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      ...
      <button @click="openAddModal" class="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition-colors">
    ...
    <div class="flex items-center gap-3 mb-4">
      <div class="relative flex-1 max-w-xs">
        ...
        <input v-model="search" type="text" placeholder="Search employees..." class="w-full pl-9 pr-3 py-2 text-sm border border-zinc-200 dark:border-zinc-700 rounded-lg bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
      </div>
          ...
          <tr v-for="emp in filteredEmployees" :key="emp.id" class="border-b border-zinc-100 dark:border-zinc-800 last:border-0 hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
            <td class="px-4 py-3">
              <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-900 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-medium text-xs flex-shrink-0">{{ getInitials(emp.name) }}</div>
                <div>
            ...
            <td class="px-4 py-3">
              <span :class="['text-xs px-2 py-1 rounded-md font-medium', emp.status === 'active' ? 'bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400']">{{ emp.status }}</span>
            </td>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4" @click.self="closeModal">
      <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl w-full max-w-md p-6">
        ...
        <div class="flex gap-3 mt-6">
          <button @click="closeModal" class="flex-1 py-2 text-sm font-medium text-zinc-700 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-700 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">Cancel</button>
          <button @click="saveEmployee" class="flex-1 py-2 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition-colors">{{ editingEmployee ? 'Save Changes' : 'Add Employee' }}</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="deletingEmployee" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4" @click.self="deletingEmployee = null">
      <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl w-full max-w-sm p-6">
        ...
        <div class="flex gap-3">
          <button @click="deletingEmployee = null" class="flex-1 py-2 text-sm font-medium text-zinc-700 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-700 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">Cancel</button>
          <button @click="doDelete" class="flex-1 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors">Delete</button>
        </div>
      </div>
    </div>
  </div>
...
const filteredEmployees = computed(() => {
  return dataStore.employees.filter(emp => {
    const matchesSearch = emp.name.toLowerCase().includes(search.value.toLowerCase()) || emp.email.toLowerCase().includes(search.value.toLowerCase())
...
const emptyForm = () => ({ name: '', email: '', position: '', department: '', role: 'employee', salary: 0, status: 'active' })
const form = ref(emptyForm())

function openAddModal() { editingEmployee.value = null; form.value = emptyForm(); showModal.value = true }
function openEditModal(emp) { editingEmployee.value = emp; form.value = { ...emp }; showModal.value = true }
...
function saveEmployee() {
  ...
  if (editingEmployee.value) {
    ...
    dataStore.addEmployee({ ...form.value, id: newId, password: 'emp123', phone: '', joinDate: new Date().toISOString().split('T')[0], avatar: null })
  }
  closeModal()
}

function confirmDelete(emp) { deletingEmployee.value = emp }
function doDelete() { dataStore.deleteEmployee(deletingEmployee.value.id); deletingEmployee.value = null }
</script>